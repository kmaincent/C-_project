...
{
    livre::save(infile);
    infile<<editeur<<endl<<nb_article<<endl;
    for(int i=0;i<nb_article;i++)
    {
        infile<<nom_article[i]<<endl;
    }
}
...
void revue::show() const
{
    ...
    for(int i=0;i<nb_article;i++)
    {
        cout<<"Article "<<i<<" : "<<nom_article[i]<<endl;
    }
}
...
void revue::load(std::istream &file)
{
    ...
    cout << "Quel est le nom de l'article suivant?" << endl;
    for (int i=0;i<nb_article; i++)
    {
        do{
            getline( file, tampon);
        }while(tampon.size()==0);
        nom_article.push_back(tampon);
    }
}
...
bool revue::search(std::string str) const
{
    ...
    for (int i= 0; i<nb_article; i++)
    {
        if (nom_article[i].find(str)!=string::npos)
            return true;
    }
    return false;
